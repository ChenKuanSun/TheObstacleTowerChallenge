...
class OTCPreprocessing(object):
    ...
    def __init__(self, environment):
        ...
        self.table_action = action_table.create_rainbow_old_action_table()
    ...
    def reset(self):
        ...
        observation = cv2.resize(
            observation[0], (84, 84), interpolation=cv2.INTER_CUBIC)
        ...
        if (len(observation.shape) > 2):
            observation = cv2.cvtColor(cv2.convertScaleAbs(
                observation, alpha=(255.0 / 1.0)), cv2.COLOR_BGR2GRAY)
    ...
    def step(self, action):
        ...
        for _ in self.table_action[int(action)]:
            ...
            self.stage_reward, self.previous_stage_time_remaining, self.game_over, self.stage_clear = reward_function.compute(
                                                                                                            done=self.game_over,
        ...
        if (len(observation.shape) > 2):
            observation = cv2.cvtColor(cv2.convertScaleAbs(cv2.resize(
                observation, (84, 84), interpolation=cv2.INTER_CUBIC), alpha=(255.0 / 1.0)), cv2.COLOR_BGR2GRAY)
        return observation, self.stage_reward, self.game_over, info, self.reward
